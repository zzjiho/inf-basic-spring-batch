package com.example.infspringbatch.job.MultipleStep;
...
public class MultipleStepJobConfig {
    ...
    public Job multipleStepJob(Step multipleStep1, Step multipleStep2, Step multipleStep3) {
        return jobBuilderFactory.get("multipleStepJob")
                ...
                .start(multipleStep1) // 여러개 스텝 등록해서 순차적으로 사용하기
                .next(multipleStep2)
                .next(multipleStep3)
                .build();
    }
    ...
    public Step multipleStep2() {
        return stepBuilderFactory.get("multipleStep2")
                .tasklet((contribution, chunkContext) -> {
                    ...
                    ExecutionContext executionContext = chunkContext // ExcutionContext를 통해 step to step 데이터 전달
                            .getStepContext()
                            .getStepExecution()
                            .getJobExecution()
                            .getExecutionContext();
